...
def _render_timer(section: str, placeholder):
    ...
    placeholder.markdown(
        f"⏱️ Waktu tersisa: **:{color}[{_format_time(remaining)}]**"
    )
    return remaining
...
def _render_init():
    ...
    paused = next(
        (s for s in paused_sessions if s.get("status") == "paused"), None
    )
...
def _render_generating():
    ...
    try:
        ...
        st.error(
            "❌ Gagal menyiapkan soal simulasi. Silakan coba lagi.\n\n"
            f"Detail: {str(e)}"
        )
...
def _render_listening():
    ...
    with col1:
        if st.button("✅ Selesai Listening — Lanjut ke Pause Screen",
...
def _render_pause_screen(after_section: int):
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button(f"▶️ Lanjut ke {next_section_name}",
                     use_container_width=True, type="primary"):
...
def _render_structure():
    ...
    col1, col2 = st.columns([2, 1])
    with col1:
        if st.button("✅ Selesai Structure — Lanjut ke Pause Screen",
                     use_container_width=True, type="primary"):
...
def _render_reading():
    ...
    if st.button("✅ Selesai Reading — Hitung Skor",
                 use_container_width=True, type="primary"):
...
def _render_completed():
    ...
    with col1:
        st.metric(
            ...
            help=(
                f"Raw: {score.get('listening_raw', 0)} / "
                f"{cfg['listening']['total']} soal"
            ),
        )
    ...
    with col2:
        st.metric(
            ...
            help=(
                f"Raw: {score.get('structure_raw', 0)} / "
                f"{cfg['structure']['total']} soal"
            ),
        )
    ...
    with col3:
        st.metric(
            ...
            help=(
                f"Raw: {score.get('reading_raw', 0)} / "
                f"{cfg['reading']['total']} soal"
            ),
        )
    ...
    if analytics.get("insight"):
        ...
        st.info(
            "📊 Kumpulkan minimal 3 simulasi untuk melihat analisis tren skor."
        )
