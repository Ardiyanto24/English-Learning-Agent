...
from database.repositories.speaking_repository import (
    save_speaking_session, save_exchange, update_exchange_transcript,
    update_speaking_scores, rebuild_transcript_from_db,
)
...
def _run_conversation_turn(
    ...
    if decision == "stop":
        return {"decision": "stop", "next_prompt": None,
    ...
    return {"decision": decision, "next_prompt": next_prompt,
            "reason": assessment.get("reason", "")}
...
def _state_oral_timer():
    ...
    if not st.session_state.get(recording_key):
        if st.button("🔴 Mulai Presentasi", type="primary",
                     key="sp_oral_start"):
...
def _state_oral_text_fallback():
    ...
    if st.button("Submit ✓", type="primary",
                 disabled=not text, key="sp_oral_text_submit"):
        session_id   = _get("session_id")
        prompt_text  = _get("prompt_text", "")
        full_history = _get("full_history", [])
...
def _try_recover_session():
    ...
    if not recovered or not recovered.get("is_recoverable"):
        st.warning(
            "⚠️ Sesi tidak bisa dilanjutkan setelah refresh. "
            "Silakan mulai sesi baru."
        )
        _reset()
        st.rerun()
        return
